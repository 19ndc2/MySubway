...
export default function Diagram() {
  ...
  return (
    <div ref={containerRef} style={{ width: "100%", height: "100vh", display: "flex", flexDirection: "column" }}>
      {/* Toolbar */}
      <div style={{ marginBottom: "10px", display: "flex", flexWrap: "wrap", gap: "0px", alignItems: "center" }}>
        {/* Group 1: Make Diagram & Import Image */}
        <div style={{ display: "flex", gap: "10px", alignItems: "center", paddingRight: "20px" }}>
          <button onClick={handleMakeDiagram} disabled={isConverting || (!lines.length && !texts.length && !images.length)}>
            {isConverting ? "Converting..." : "Make Diagram"}
          </button>
          <label>
            <input type="file" accept="image/*" onChange={handleImportImage} style={{ display: "none" }} />
            <span style={{ cursor: "pointer", padding: "5px", border: "1px solid #ccc" }}>Import Image</span>
          </label>
        </div>

        {/* Divider */}
        <div style={{ width: "1px", height: "24px", backgroundColor: "#ccc", marginRight: "20px" }} />

        {/* Group 2: Clear, Undo, Redo */}
        <div style={{ display: "flex", gap: "10px", alignItems: "center", paddingRight: "20px" }}>
          <button onClick={clearCanvas} style={{ backgroundColor: "#ffcccc" }}>Clear</button>
        ...
        <div style={{ display: "flex", gap: "10px", alignItems: "center", paddingRight: "20px" }}>
          <button onClick={() => setTool("pen")} style={{ backgroundColor: tool === "pen" ? "#ddd" : "white" }}>Pen</button>
          <button onClick={() => setTool("eraser")} style={{ backgroundColor: tool === "eraser" ? "#ddd" : "white" }}>Eraser</button>
          <button onClick={() => setTool("select")} style={{ backgroundColor: tool === "select" ? "#ddd" : "white" }}>Text</button>
        </div>
        ...
        <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
          {swatches.map((color, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: "0px", position: "relative" }}>
              <div onClick={() => setCurrentColor(color)} style={{ width: 24, height: 24, backgroundColor: color, border: currentColor === color ? "2px solid black" : "1px solid #ccc", cursor: "pointer" }} />
              <button onClick={() => setOpenColorPicker(openColorPicker === i ? null : i)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: "10px", padding: "0 2px", marginLeft: "-2px" }}>▼</button>
              {openColorPicker === i && (
      ...
      {editingIndex !== null && <input type="text" autoFocus value={editingValue} onChange={e => setEditingValue(e.target.value)} onBlur={finishEditing}
        ...
        style={{ position: "absolute", left: texts[editingIndex].x, top: texts[editingIndex].y + 50, zIndex: 1000 }}
      />}
      ...
      {convertedImage && <div style={{ marginTop: 10 }}>
        ...
        <button onClick={handleSaveConverted}>Save Converted</button>
      </div>}
    </div>
  );
}